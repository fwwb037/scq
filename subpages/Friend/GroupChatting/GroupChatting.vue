...
<script>
	...
	export default {
		...
		created() {
			http.get('/youngster/getHistoryDialogues', {
				...
				if(res.data.data.length<10){
					this.ajax.rows=res.data.data.length
				}
			...
			joinHistoryMsg(){
				let join = ()=>{
					...
					for(let i = startIndex; i < endIndex; i++){
						arr.push({
							"id":dl[i].userId,	// 消息的ID
							"content":dl[i].dialogueContent,	// 消息内容
							"type":this.$store.state.username === dl[i].userId ? 1 : 0	,// 此为消息类别，设 1 为发出去的消息，0 为收到对方的消息,
							"pic":dl[i].headImg	// 头像
						})
					}
