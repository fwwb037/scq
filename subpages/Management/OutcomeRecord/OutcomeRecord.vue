		...
		<view style=" margin-left: 18px;margin-right: 18px;">
			<view >
				<u-swipe-action @click="ooo">
					<u-swipe-action-item  v-for="(item, index) in outcomeList" :options="options5" @click="onClick($event,item.outcomeId)"> 
						<view >
							<view style="height: 50px; margin-left: 5px;margin-top:3%;">
								<tm-icons :name="item.typeIcon"></tm-icons>
								<text style="font-size: 20px;">{{item.outcomeName}}</text><br/>
		...
		<tm-poup v-model="show_poup" :position="pos" :height="300">
			<view>
					<tm-form @submit="submit" ref="formData" @request="success" method="post">
						<tm-input name="title" required title="修改支出名称" v-model="reqData.title"></tm-input>
						
						<tm-input name="price" required title="修改支出金额" input-type="digit" v-model="reqData.price"
							suffix="万"></tm-input>
	...
	export default {
		...
		onLoad() {
			setTimeout(() => {
				...
				http.get('/youngster/getAllOutcomes', {}).then(res => {
					console.log(res)
		...
		methods: {
			...
			onClick(e,t) {
				...
				if (e.index === 1) {
					...
					console.log('删除')
					http.put('/youngster/deleteOutcomeRecord', {
						"typeId":t,
