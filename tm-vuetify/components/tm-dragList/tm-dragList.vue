				...
				>
					<view v-if="item['icon']" class="flex-shrink pl-32 fulled-height flex-center">
						<tm-icons :black="black_tmeme" :color="item['color']||'black'" :fllowTheme="fllowTheme" dense
							:name="item['icon']" :size="75"></tm-icons>

					</view>
					
					
					<text style="font-size: 15px;width: 70%;" :class="[black_tmeme?'bk':'']">{{item.text}}</text>
					
					<text style="font-size:12px" :class="[black_tmeme?'bk':'']">￥{{item.cost}}</text>
